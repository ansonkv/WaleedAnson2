...
class FilterBrandAdapter(
    private var brandValueList: List<FilterResponse.BrandValue>,
    private var context: Context
) : RecyclerView.Adapter<FilterBrandAdapter.FilterCategoryValueVH>() {
    ...
    inner class FilterCategoryValueVH(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(data: FilterResponse.BrandValue) {
    ...
    override fun onBindViewHolder(holder: FilterCategoryValueVH, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            if (selectedPositions.contains(position)) {
                ...
                selectedValues.remove(brandValueList[position].brandID)
                notifyItemChanged(position)
            } else {
                selectedPositions.add(position)
                selectedValues.add(brandValueList[position].brandID)
                notifyItemChanged(position)
            }
